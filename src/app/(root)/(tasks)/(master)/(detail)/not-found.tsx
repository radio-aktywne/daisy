...
import { TasksDetailNotFoundView } from "./not-found.view";

async function getTitle({}: NotFoundMetadataUtilityInput = {}) {
  return msg({ message: "Not Found • daisy" });
}
...
export default async function TasksDetailNotFound({}: NotFoundInput) {
  await connection();

  return (
    <>
      <Metadata title={await getTitle()} />
      <TasksDetailNotFoundView />
    </>
  );
}
