...
export const authConfig = {
  ...
  cookies: {
    callbackUrl: {
      ...
      options: {
        domain: process.env.DAISY__COOKIES__DOMAIN || undefined,
      },
    },
    csrfToken: {
  ...
  debug: (process.env.DAISY__DEBUG || "true") === "true",
  pages: {
    error: "/auth/error",
    newUser: "/auth",
    signIn: "/auth/login",
  ...
  secret: splitSecrets(process.env.DAISY__SECRETS__AUTH || "secret"),
  session: {
